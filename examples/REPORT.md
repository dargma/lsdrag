...
## 5-c. 종합 검증 — 표·그림(이미지)·둘 동시 읽기

**A. 멀티모달 단일 호출(이미지+표 동시)**: crop figure 이미지 + table markdown을 GPT-4.1 mini에 한 번에 전달
→ 이미지의 두 상태("Open/Exclusive Access")와 표 첫 행("Gathering")을 **둘 다 읽고 관계까지** 설명. ✅

**B. 에이전트 루프에서 figure+table 동시 확보** — 처음엔 **실패**(중요 발견):
- 에이전트가 figure를 못 불러옴(`image_read` 0회). 원인: ① figure 번호 없음 → 못 찾음 ② 에이전트는 페이지
  라벨(E2-2801)을 보는데 `page` 쿼리는 split-local 번호(1~26)와 비교 → 0 hits, max_loops까지 헤맴.
- **수정**: ① `page_index_search`의 `page`가 **인쇄 라벨도 매칭**(`page="E2-2801"`) ② 텍스트가 걸린 페이지의
  **figure 이미지를 자동 노출** → 에이전트가 image_read하도록. 스키마 설명도 갱신.
- **재검증(라이브)**: 질의 "page E2-2801 다이어그램 읽고 + 표의 Gathering 설명" →
  `page_index_search(page='E2-2801')`(라벨 매칭) → figure 노출 → **`image_read`로 다이어그램 읽음** →
  `keyword/read_chunk`로 표(Gathering) → 답변이 **그림(이미지)과 표를 통합**. ✅
- 회귀 고정: 라벨 조회·figure 자동 노출 테스트(`tests/test_retrieval.py`).

**결론**: 표 단독·그림(이미지) 단독·**이미지+표 동시** 모두 읽고 함께 불러와짐을 실제 데이터·실제 API로 확인.
